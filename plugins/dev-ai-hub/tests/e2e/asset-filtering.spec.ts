...
test.describe('Asset Filters', () => {
  ...
  test('SearchField has the correct placeholder text', async ({ page }) => {
    await expect(
      ...
      'placeholder',
      'Search assets by name, description or content…',
    );
  });
  ...
  test('type filter dropdown is rendered with default "All Types"', async ({
    page,
  }) => {
  ...
  test('"Instructions" option filters to instruction assets only', async ({
    page,
  }) => {
  ...
  test('"Workflows" option filters to workflow assets only', async ({
    page,
  }) => {
    await page.getByRole('button', { name: 'Filter by type' }).click();
  ...
  test('"All Types" option clears the type filter', async ({ page }) => {
    ...
    await expect(
      page.getByText(`${DEV_ASSETS.length} assets found`),
    ).toBeVisible();
  });
  ...
  test('"Claude Code" option shows only claude-code-compatible assets', async ({
    page,
  }) => {
  ...
  test('"Cursor" option shows only cursor-compatible assets', async ({
    page,
  }) => {
    await page.getByRole('button', { name: 'Filter by AI tool' }).click();
  ...
  test('opening tags dropdown shows available tags as checkboxes', async ({
    ...
    await expect(
      page.getByRole('checkbox', { name: 'typescript' }),
    ).toBeVisible();
  });
  ...
  test('trigger label shows selected count when tags are active', async ({
    page,
  }) => {
    await page.getByRole('button', { name: 'Filter by tags' }).click();
    await page.getByRole('checkbox', { name: 'typescript' }).check();
