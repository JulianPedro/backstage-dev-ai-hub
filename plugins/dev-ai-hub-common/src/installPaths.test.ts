...
describe('getInstallPath', () => {
  describe('claude-code conventions', () => {
    it('instruction → .claude/rules/', () => {
      expect(getInstallPath('instruction', 'claude-code', 'My Rule')).toBe(
        '.claude/rules/my-rule.md',
      );
    });
    it('agent → .claude/agents/', () => {
      expect(getInstallPath('agent', 'claude-code', 'My Agent')).toBe(
        '.claude/agents/my-agent.md',
      );
    });
    it('skill → .claude/skills/<name>/SKILL.md', () => {
      expect(getInstallPath('skill', 'claude-code', 'My Skill')).toBe(
        '.claude/skills/my-skill/SKILL.md',
      );
    });
    it('workflow → .claude/workflows/', () => {
      expect(getInstallPath('workflow', 'claude-code', 'My Workflow')).toBe(
  ...
  describe('github-copilot conventions', () => {
    ...
    it('agent → .github/agents/*.agent.md', () => {
      expect(getInstallPath('agent', 'github-copilot', 'My Agent')).toBe(
  ...
  describe('google-gemini conventions', () => {
    it('instruction → GEMINI.md (ignores name)', () => {
      expect(getInstallPath('instruction', 'google-gemini', 'Anything')).toBe(
        'GEMINI.md',
      );
    });
    it('agent → GEMINI.md (ignores name)', () => {
      expect(getInstallPath('agent', 'google-gemini', 'My Agent')).toBe(
        'GEMINI.md',
      );
    });
    it('workflow → .gemini/workflows/', () => {
      expect(getInstallPath('workflow', 'google-gemini', 'My Workflow')).toBe(
        '.gemini/workflows/my-workflow.md',
      );
    });
  });
  ...
  describe('cursor conventions', () => {
    it('instruction → .cursor/rules/*.mdc', () => {
      expect(getInstallPath('instruction', 'cursor', 'My Rule')).toBe(
        '.cursor/rules/my-rule.mdc',
      );
    });
    it('skill → .cursor/skills/<name>/SKILL.md', () => {
      expect(getInstallPath('skill', 'cursor', 'My Skill')).toBe(
  ...
  describe('slug normalization', () => {
    it('lowercases the name', () => {
      expect(getInstallPath('instruction', 'claude-code', 'UPPERCASE')).toBe(
        '.claude/rules/uppercase.md',
      );
    });
    it('replaces spaces with hyphens', () => {
      expect(
        getInstallPath('instruction', 'claude-code', 'My Fancy Rule'),
      ).toBe('.claude/rules/my-fancy-rule.md');
    });
    it('removes special characters', () => {
      expect(getInstallPath('instruction', 'claude-code', 'Rule (v2)!')).toBe(
        '.claude/rules/rule-v2.md',
      );
    });
    it('preserves underscores', () => {
      expect(getInstallPath('instruction', 'claude-code', 'my_rule')).toBe(
        '.claude/rules/my_rule.md',
      );
    });
  ...
  describe('unknown tool', () => {
    it('uses default convention for unrecognised tool identifier', () => {
      expect(getInstallPath('instruction', 'unknown-tool', 'My Rule')).toBe(
        '.ai/instructions/my-rule.md',
      );
    });
  });
});
...
describe('getInstallPathsForAsset', () => {
  it('returns paths only for the listed tools', () => {
    const paths = getInstallPathsForAsset(
      'instruction',
